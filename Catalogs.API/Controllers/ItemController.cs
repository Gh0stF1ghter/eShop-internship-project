...
using Catalogs.Application.CQRS.Queries.ItemQueries.GetAllItems;
...
namespace Catalogs.API.Controllers
{
    ...
    public class ItemController(ISender sender) : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllItemsAsync([FromQuery] ItemParameters itemParameters, CancellationToken cancellationToken = default)
        {
            var (items, metaData) = await _sender.Send(new GetAllItemsQuery(itemParameters), cancellationToken);
        ...
        [HttpDelete("types/{typeId}/items/{id}")]
        [Authorize(Roles = UserRoles.Admin)]
